<template>
  <div class="w-full bg-white rounded-xl shadow-lg justify-center py-10 flow-root h-32">
    <div class="float-left w-60">
      <svg title="Click to Show Fiat/Local Values" @click="showFiat = !showFiat" class="float-left mx-3 cursor-pointer" width="40" height="41" viewBox="0 0 50 51" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <DashboardAssetsValueType :show-fiat="showFiat" title="Total Value" :value="totalValue.usd" :fiat-value="totalValue.fiat" percChange="10"/>
    </div>

    <div class="flex grid grid-cols-2">
      <DashboardAssetsValueType :show-fiat="showFiat" title="Total Profit" :value="totalProfit.usd" :fiat-value="totalProfit.fiat" percChange="18"/>

      <DashboardAssetsValueType :show-fiat="showFiat" title="Total Loss" :value="totalLoss.usd" :fiat-value="totalLoss.fiat" percChange="4"/>
    </div>
  </div>
</template>
